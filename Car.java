...
/** Class Car */
public class Car {
  // each car has it's own hashmap which stores all passengers in the car.
  // key=name, value=passenger object
  ...
  public void addPassenger(String name, String description, String dialogue) {
    Passenger p = new Passenger(name, description, dialogue); // make a new passenger and temporarily label it passenger
                                                              // "p"
    carPassengers.put(p.name, p); // add the new passenger to the hashmap. from here-on-out it will be labelled by
                                  // its name, not p, since the hashmap key=name
  }
